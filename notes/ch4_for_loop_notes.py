...
for num in range(10):       # range(n) creates an n digit sequence [0-n)
    print("Hello, world.")      # body runs for each element in the range

# Display numbers 1-10 using a for loop
input("\nPress enter to begin.")
for num in range(1, 11):    # range(x, y) creates a sequence [x, y); includes x, excludes y
    print(num)                          # num is a variable whose value is each element in the sequence, one at a time

print(num)                              # the variable num exists outside the for loop (the value is 10)
...
#   Count by 2's
input("\nPress enter to begin.")
for i in range(0, 21, 2):       # range(x, y, z) creates a sequence [x, y) where each element is incremented by z
    print(i)
